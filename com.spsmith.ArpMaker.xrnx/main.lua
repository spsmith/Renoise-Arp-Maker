...
function count_instruments()
    ...
    for _ in pairs(renoise.song().instruments) do
...
function count_phrases(instrument)
    local num_phrases = 0
    for _ in pairs(instrument.phrases) do
...
function construct_note(phrase, phrase_number, note_number)
    ...
    -- get the line
    local two_note_arp =
        math.floor(phrase_number / 16) == 0 and settings.length.value % 3 == 0 and settings.mode.value ~= 3 -- special settings for arps with only two pitches
    ...
    note_column.panning_value = 0x40 +
                                    (stereo_mul * lerp(-settings.stereo_spread.value, settings.stereo_spread.value, t))
...
function show_tool_window()
    ...
    -- valueboxes
    local instrument_valuebox = vb:row{vb:text{
    ...
    local instrument_namebox = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
    ...
    local offset_valuebox = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Sample Offset"
    }, vb:valuebox{
    ...
    local length_valuebox = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Notes"
    }, vb:valuebox{
    ...
    local mode_switch = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Mode"
    }, vb:switch{
        bind = settings.mode,
    ...
    local vol_min_slider = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Volume (start)"
    }, vb:slider{
    ...
    local vol_max_slider = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Volume (end)"
    }, vb:slider{
    ...
    local stereo_slider = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Stereo Spread"
    }, vb:slider{
    ...
    local stereo_direction = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
    ...
    local glide_slider = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Glide"
    }, vb:slider{
        bind = settings.glide,
    ...
    local speed_valuebox = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Speed"
    }, vb:valuebox{
        bind = settings.speed,
    ...
    local loop_checkbox = vb:row{vb:text{
        width = TEXT_ROW_WIDTH,
        text = "Loop"
    }, vb:checkbox{
        bind = settings.loop
    ...
    control_example_dialog = renoise.app():show_custom_dialog("ArpMaker", dialog_content)
